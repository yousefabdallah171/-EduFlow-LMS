    ...
    <AdminShell title={t("admin.notifications.title")} description={t("admin.notifications.desc")}>
        ...
        <div className="space-y-3">
          {isLoading ? (
            ...
            (data?.templates ?? []).map((tmpl) => (
              <div key={tmpl.id} className="dashboard-panel cursor-pointer rounded-[24px] p-4 transition-all hover:bg-surface2" style={{ backgroundColor: selected?.id === tmpl.id ? "var(--color-brand-muted)" : undefined, borderColor: selected?.id === tmpl.id ? "var(--color-brand)" : undefined }} onClick={() => setSelected({ ...tmpl })}>
          ...
          <div className="space-y-4">
            <div className="dashboard-panel p-5 space-y-3">
              ...
              <div className="flex gap-2">
                <button onClick={() => void updateMut.mutateAsync(selected)} disabled={updateMut.isPending} className="rounded-xl px-5 py-2 text-sm font-bold text-white transition-all hover:opacity-95 disabled:opacity-60" style={{ background: "var(--gradient-brand)" }} type="button">{t("actions.save")}</button>
            ...
            <div className="dashboard-panel p-5">
              <p className="mb-3 text-xs font-bold uppercase tracking-[0.16em]" style={{ color: "var(--color-text-muted)" }}>Preview</p>
              <div className="text-sm" style={{ color: "var(--color-text-primary)" }} dangerouslySetInnerHTML={{ __html: selected.bodyHtml }} />
            </div>
          </div>
        ) : (
          <div className="dashboard-panel flex items-center justify-center">
      ...
      {broadcasting && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="rounded-[28px] border p-6 max-w-sm w-full mx-4" style={{ backgroundColor: "var(--color-surface)", borderColor: "var(--color-border)" }}>
            ...
            <div className="flex gap-2">
              <button onClick={() => void broadcastMut.mutateAsync()} disabled={broadcastMut.isPending} className="flex-1 rounded-xl py-2.5 text-sm font-bold text-white transition-all hover:opacity-95 disabled:opacity-50" style={{ background: "var(--gradient-brand)" }} type="button">{broadcastMut.isPending ? "Sending..." : "Send"}</button>
