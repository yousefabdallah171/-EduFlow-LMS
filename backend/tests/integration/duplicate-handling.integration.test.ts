...
const adminEmail = "duplicate-policy-admin@example.com";
...
describe("Upload duplicate handling", () => {
  it("rejects duplicates when policy is SKIP", async () => {
    ...
    expect(response.body.error).toBe("UPLOAD_DUPLICATE_DETECTED");
  });

  it("allows duplicates when policy is REPLACE", async () => {
    const token = await loginAsAdmin();
  ...
  it("allows duplicates when policy is UPLOAD_AS_NEW", async () => {
    const token = await loginAsAdmin();
